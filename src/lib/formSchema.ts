...
export const entryFormSchema = z
  .object({
    ...
    kind: z.enum(['operating', 'deadhead', 'ground']),
  ...
  .refine(
    (values) =>
      values.kind === 'ground'
        ? values.groundDutyTime !== ''
        : values.blockTime !== '' || values.deadheadTime !== '',
    {
  ...
  .refine(
    (values) => {
      if (values.kind === 'ground') return true;
...
export function formValuesToEntry(values: EntryFormValues): NewLogEntry {
  ...
  return {
    ...
    dayMinutes: kind === 'ground' ? 0 : minutes(values.dayTime),
    nightMinutes: kind === 'ground' ? 0 : minutes(values.nightTime),
